...
//Create analysis.csv to analysis data

    if ( (analysis = fopen ("analysis.csv", "w+")) == NULL) 
    {
    ...
    else
    {
        ...
        fseek(analysis, 0, SEEK_SET) ;

        // Gradient Descent
        for (it=0; it<TRAINING_TIMES; it++)  
        {
            ...
            back_prop(i);
            update_weights();
            get_MAE(i+1, absolute, it+1);
            get_MSE(i+1, plus, it+1);
            }
...
void compute_cost(int i)
{
    ...
    for(j=0;j<num_neurons[num_layers-1];j++)
    {
        ...
        tcost = tcost + cost[j];
        plus=tmpcost*tmpcost;
        absolute=fabs(tmpcost);
    }   
...
void get_MAE(int i, double absolute, int it)
{
    int j;

    for(j=0;j<num_neurons[num_layers-1];j++)
    {
        sum_mae+=absolute; //Get the sum of the absolute value in each iteration
    }

    if ((i)%(num_training_ex)==0) // compute every 4 datas
    {
        sum_mae = sum_mae/it; //Get MAE
        printf("Mean Absolute Error is: %f \n",sum_mae); 
        fprintf(analysis,"MAE,%f,",sum_mae);
        sum_mae=0;
    } 
...
void get_MSE(int i, double plus,int it)
{
    int j;

    for(j=0;j<num_neurons[num_layers-1];j++)
    {
        sum_mse+=plus; //Get the sum of the plus value in each iteration
    }

    if ((i)%(num_training_ex)==0) // compute every 4 datas
    {
        sum_mse = sum_mse/it; //Get MSE
        printf("Mean Square Error is: %f \n",sum_mse); 
        fprintf(analysis,"MSE,%f\n",sum_mse);
        sum_mse=0;
    } 
